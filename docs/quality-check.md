...
1. （バックエンド）`mypy`/`flake8`未導入（`ruff`は導入済み）
2. （バックエンド）`@app.on_event("startup")`が非推奨API（`lifespan`への移行が将来的に必要）
3. （バックエンド）パスワードバリデーションのテストカバレッジに1点抜け（8文字未満かつ複雑さ要件も満たさないケースのテストはないが、`min_length=8`制約で確実に弾かれるため実害はなし）
4. （DB）`Schedule`モデルに`expense`への逆参照`relationship`がない（現状未使用のため実害なし）
